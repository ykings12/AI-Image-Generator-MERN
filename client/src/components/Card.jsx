...
const Card = ({ _id, name, prompt, photo, onCardClick }) => {
  return (
    <div className='rounded-xl group relative shadow-card hover:shadow-cardhover card border border-transparent hover:border-blue-500'>
      <img 
        className='w-full h-auto object-cover rounded-xl cursor-pointer'
        src={photo}
        alt={prompt}
        onClick={() => onCardClick && onCardClick({ _id, name, prompt, photo })}
      />
